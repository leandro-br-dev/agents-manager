...
/**
 * Comprehensive Test Script for Agent Linking Fixes
 *
 * This script tests:
 * 1. Agent name display fix
 * 2. Auto-link on agent creation
 * 3. Auto-unlink on agent deletion
...
try {
  const projectsPage = fs.readFileSync('dashboard/src/pages/ProjectsPage.tsx', 'utf8');
  const workspacesApi = fs.readFileSync('dashboard/src/api/workspaces.ts', 'utf8');
  const workspacesRoute = fs.readFileSync('api/src/routes/workspaces.ts', 'utf8');
...
try {
  ...
  const result = execSync(`cd api && node -e "${dbCheckScript}"`, { encoding: 'utf8' });
...
try {
  ...
  const testPaths = [
    '/root/projects/agents-manager/projects/test/agent-coder',
    '/root/projects/agents-manager/projects/test/agents/planner',
    '/root/projects/agents-manager/projects/my-project/frontend-dev',
    '/root/projects/agents-manager/projects/simple/coder'
  ];
...
try {
  const distExists = fs.existsSync('dashboard/dist');
  if (distExists) {
