...
describe('CreatePlanForm', () => {
  ...
  it('renders form with all required fields', () => {
    ...
    expect(screen.getByLabelText(/Agent Workspace/)).toBeInTheDocument();
  ...
  it('renders initial task with default values', () => {
    ...
    const workspaceSelect = screen.getByLabelText(/Agent Workspace/) as HTMLSelectElement;
  ...
  it('shows validation errors for empty task fields', async () => {
    const user = userEvent.setup();
    renderWithProviders(<CreatePlanForm />);
  ...
  it('updates cwd to workspace project root when workspace is selected without environment', async () => {
    ...
    const workspaceSelect = screen.getByLabelText(/Agent Workspace/);
  ...
  it('shows environment details after selection', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('/root/project1')).toBeInTheDocument();
      expect(screen.getByText('Agent config:')).toBeInTheDocument();
      expect(screen.getByText('/root/workspace1')).toBeInTheDocument();
    });
  });
});
