...
# 1. Check that llm-router is running
echo "1. Checking llm-router..."
if curl -s http://localhost:8083/health > /dev/null 2>&1; then
  echo "   ✓ llm-router is running on :8083"
else
  echo "   ⚠ llm-router NOT running on :8083 (agents will use default auth)"
fi
...
export AGENT_CLIENT_PATH=/root/projects/agents-manager/projects
...
SESSION_RESPONSE=$(curl -s -X POST \
  ...
  -d '{"name":"chat-fix-test","workspace_path":"/root/projects/agents-manager/projects/agents-manager/agents/planner"}' \
